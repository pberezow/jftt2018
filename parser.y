...
string use_register() {
	...
	cout << "BLAD PRZY DOBIERANIU REJESTROW";
	return "BLAD PRZY DOBIERANIU REJESTROW";
}
...
struct block* get_next_iter(struct ast* ID_node, struct ast* value_node) {
	...
	struct indirect_code* code = handle_value(value_node, reg_gen);
	iter_block->codes.push_back(code);

	struct variable* reg_B = newvariable("REG", reg_gen, "", 0);
	code = newindirect_code("@STORE", reg_B, iterator);
	iter_block->codes.push_back(code);

	free_register(reg_gen);
...
void _handle_lt(struct ast* condition_node, struct variable* end_label, vector<struct indirect_code*>* vec) {
	string reg_gen = use_register();
	struct variable* reg_B = newvariable("REG", reg_gen, "", 0);
	reg_gen = use_register();
	struct variable* reg_C = newvariable("REG", reg_gen, "", 0);
	
	struct indirect_code* a = handle_value(condition_node->s_1, reg_B->id1);
	struct indirect_code* b = handle_value(condition_node->s_2, reg_C->id1);
	(*vec).push_back(a);
	(*vec).push_back(b);
	...
	(*vec).push_back(jzero);
	free_register(reg_B->id1);
	free_register(reg_C->id1);
}
...
void _handle_eq(struct ast* condition_node, struct variable* end_label, vector<struct indirect_code*>* vec) {
	...
	struct variable* reg_C = newvariable("REG", reg_gen, "", 0);
	reg_gen = use_register();
	struct variable* reg_D = newvariable("REG", reg_gen, "", 0);
...
struct block* handle_for_to(struct ast* for_to_node) {
	...
	struct indirect_code* b = handle_value(for_to_node->s_3, reg_C->id1);
	...
	if(check_if_stolen(reg_B->id1) == 1) { // reg iteratora ukradziony
		...
		iter_block->codes.pop_back();
		iter_block->codes.push_back(load_iter);
		iter_block->codes.push_back(lab_loop);
		condition_block->codes[0] = newindirect_code("EMPTY", reg_B, NULL);
		struct indirect_code* inc = newindirect_code("INC", reg_B, NULL);
		tmp->codes.push_back(inc);
	}


	if(check_if_stolen(reg_C->id1) == 0) { // reg licznika nie ukradziony
		iter_block->codes.pop_back();
		iter_block->codes.push_back(b);
		iter_block->codes.push_back(lab_loop);
		condition_block->codes[1] = newindirect_code("EMPTY", reg_B, NULL);
	}
...
struct block* handle_assign(struct ast* asg_node) {
	...
	handle_expression(asg_node->s_2, reg_gen, &(asg->codes));
	struct indirect_code* code = handle_store(asg_node, reg_gen);
...
void handle_expression(struct ast* exp_node, string result_reg, vector<struct indirect_code*>* vec) {
	if(exp_node->value.compare("value") == 0) {
		...
		// wyrazenie
		string reg_gen;
		struct variable* reg1 = newvariable("NONE", "", "", 0);
		struct variable* reg2 = newvariable("NONE", "", "", 0);

		if(exp_node->value.compare("%") != 0) {
			struct indirect_code* var1 = handle_value(exp_node->s_1, result_reg);
			reg_gen = use_register();
			struct indirect_code* var2 = handle_value(exp_node->s_2, reg_gen);
			(*vec).push_back(var1);
			(*vec).push_back(var2);
			reg1 = newvariable("REG", result_reg, "", 0);
			reg2 = newvariable("REG", reg_gen, "", 0);
		}
		...
		if(exp_node->value.compare("+") == 0) { // + dodawanie
			...
			free_register(reg1->id1);
			free_register(reg2->id1);
			...
			// DO OPTYMALIZACJI
			reg_gen = use_register();
			struct variable* reg3 = newvariable("REG", reg_gen, "", 0);
			reg_gen = use_register();
			struct variable* reg4 = newvariable("REG", reg_gen, "", 0);
			struct variable* lab1 = get_next_label_id();
			struct variable* lab2 = get_next_label_id();
			...
			free_register(reg1->id1);
			free_register(reg2->id1);
			free_register(reg3->id1);
			free_register(reg4->id1);
			...
			struct variable* reg4 = newvariable("REG", reg_gen, "", 0);
			reg_gen = use_register();
			struct variable* reg5 = newvariable("REG", reg_gen, "", 0);
			...
			free_register(reg3->id1);
			free_register(reg4->id1);
			free_register(reg5->id1);	

			// (*vec).push_back(div);
		} else { // % modulo
			struct indirect_code* mod = newindirect_code("@MOD", reg1, reg2);
			// (*vec).push_back(mod);
			reg2 = newvariable("REG", result_reg, "", 0);
			reg_gen = use_register();
			struct variable* reg3 = newvariable("REG", reg_gen, "", 0);
			reg_gen = use_register();
			struct variable* reg4 = newvariable("REG", reg_gen, "", 0);
...
struct block* handle_read(struct ast* read_node) {
	...
	code = handle_store(read_node, reg_gen);
	read_block->codes.push_back(code);
...
struct block* handle_write(struct ast* write_node) {
	...
	string reg_gen = use_register();
	struct indirect_code* code = handle_value(write_node->s_1, reg_gen);
